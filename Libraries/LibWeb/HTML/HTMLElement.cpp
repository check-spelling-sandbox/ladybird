...
{
    // 1. If element is not being rendered or if the user agent is a non-CSS user agent, then return element's descendent text content.
...
int HTMLElement::offset_width() const
{
    ...
    //
    //    If the element’s principal box is an inline-level box which was "split" by a block-level descendent, also
    //    include fragments generated by the block-level descendents, unless they are zero width or height.
...
void HTMLElement::attribute_changed(FlyString const& name, Optional<String> const& old_value, Optional<String> const& value, Optional<FlyString> const& namespace_)
{
    ...
    if (name == HTML::AttributeNames::contenteditable) {
        ...
        // The inert attribute is a boolean attribute that indicates, by its presence, that the element and all its flat tree descendents which don't otherwise escape inertness
...
Optional<ARIA::Role> HTMLElement::default_role() const
{
    ...
    if (local_name() == TagNames::footer || local_name() == TagNames::header) {
        // If not a descendent of an article, aside, main, nav or section element, or an element with role=article,
...
WebIDL::ExceptionOr<void> HTMLElement::hide_popover(FocusPreviousElement focus_previous_element, FireEvents fire_events, ThrowExceptions throw_exceptions, IgnoreDomState ignore_dom_state)
{
    ...
    // FIXME: 16.2. If focusPreviousElement is true and document's focused area of the document's DOM anchor is a shadow-including inclusive descendent of element, then run the focusing steps for previouslyFocusedElement; the viewport should not be scrolled by doing this step.
