    ...
    ~ScopePusher()
    {
        ...
        for (auto& it : m_identifier_groups) {
            ...
            if (m_type == ScopeType::Function && m_bound_names.contains(identifier_group_name)) {
                // NOTE: Currently parser can't determine that named function expression assignment creates scope with binding for function name so function names are not considered as candidates to be optmized in global variables access
